 ...
 *
 * This is what I refer to as an "abusive" subclass.  It subclasses OpenALAudio only so
 * that it can inherit is interface.  It completely overrides all methods and does not
 * access any fields of the subclass (because almost none of them have protected access).
 * The original OpenALAudio by Nathan Sweet is a good first attempt, but it makes a lot
 * of design decisions that make it hard to open up OpenAL features to the main engine.
 *
 * This form of subclassing is okay because we set the superclass to not allocate any
 * resources.  It only initializes the OpenAL enigine, which is a global state machine
 * (that is its own problem, but it works for us in this situation).  
 *
...
public class GDXAudio implements AudioEngine {
    ...
    /**
     * Put effect into an available slot and set the effect to a source.
     *
    ...
    private void findPaths() throws Exception {
        ...
        try {
            Class<?> clazz = LWJGLUtil.class;
            Method m = clazz.getDeclaredMethod( "getPathFromClassLoader", String.class, ClassLoader.class );
            m.setAccessible( true );
            String classloader_path = (String)m.invoke( null, libname, AL.class.getClassLoader() );
            Gdx.app.log( "LWJGL", "Classloader path is " + classloader_path );
        } catch (Exception e) {
            Gdx.app.log("LWJGL","Failed to find classpath: "+e);
            e.printStackTrace();
        }
        String[] oalPaths = LWJGLUtil.getLibraryPaths(libname, library_names, AL.class.getClassLoader());
        Gdx.app.log( "LWJGL", "Found " + oalPaths.length + " OpenAL paths" );
