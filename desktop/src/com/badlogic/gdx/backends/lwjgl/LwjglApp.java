 ...
 *
 * Modification (Walker M White): Making initialize private AND forcing it in the constructor has made it impossible
 * to extend this class without serious thread synchronization problems.  There is a reason we do not intialize in
 * constructors in CUGL. This variation of LwjglApplication exposes that one method and requires it be called manually.
 */
public class LwjglApp implements Application {
    ...
    public LwjglApp (ApplicationListener listener, String title, int width, int height) {
        this(listener, createConfig(title, width, height));
    }

    public LwjglApp (ApplicationListener listener) {
        this(listener, null, 640, 480);
    }

    public LwjglApp (ApplicationListener listener, LwjglApplicationConfiguration config) {
        this(listener, config, new LwjglGraphics(config));
    }

    public LwjglApp (ApplicationListener listener, Canvas canvas) {
        this(listener, new LwjglApplicationConfiguration(), new LwjglGraphics(canvas));
    }

    public LwjglApp (ApplicationListener listener, LwjglApplicationConfiguration config, Canvas canvas) {
        this(listener, config, new LwjglGraphics(canvas, config));
    }

    public LwjglApp (ApplicationListener listener, LwjglApplicationConfiguration config, LwjglGraphics graphics) {
        ...
        //initialize();  RAII is not a suicide pact
    }
